...
class QuranMemorizationApp:
    def __init__(self):
        self.surahs = get_surahs()
        self.hf_token = st.secrets["hf_token"]
        self.llm_helper = LLMHelper(self.hf_token)

    def run(self):
        st.title("📖 رفيق القرآن - مراجعة وحفظ وتفسير")

        surah_name = st.selectbox("اختر السورة", list(self.surahs.keys()))
        ...
        if st.button("ابدأ الإختبار"):
            ...
            for ayah_num in range(start_ayah, end_ayah + 1):
                ...
                actual_ayah = get_ayah_text(self.surahs[surah_name], ayah_num)
                tafsir = get_tafsir(self.surahs[surah_name], ayah_num)

                # Part 1: حفظ
                st.markdown("### 🧠 اختبار الحفظ")
                user_mem = st.text_area("أكمل الآية الكريمة:")
                if user_mem:
                ...
                user_tafsir = st.text_area("اشرح معنى الآية أو الكلمات:")
                if user_tafsir:
                    prompt = f"قارن التفسير التالي بالتفسير الرسمي: '{user_tafsir}'. التفسير الرسمي: '{tafsir}'. قيمه من ١٠ مع توضيح الخطأ."
                    correction = self.llm_helper.ask(prompt)
